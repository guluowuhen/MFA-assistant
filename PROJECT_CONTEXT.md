...
- 支持导出：将当前绑定导出为 JSON（包含 `name`、`mfa_url`，以及可用的 `secret`）
- 支持导入：从 JSON 导入绑定数据（兼容 `items` 或 `systems` 字段）
- 导入按 `mfa_url` 做唯一性校验：
  - 导入文件内部若有重复 `mfa_url` 会直接报错
  - 与已绑定列表冲突时，在页面展示“导入冲突处理”表格，可批量选择：
    - `全部覆盖`（保留导入）
    - `全部保留`（保留已绑定）
  - 也可逐条选择后点击“应用导入”
...
6. 如需迁移，可在“已绑定系统”使用导出/导入：
   - 无冲突时直接导入
   - 有重复 `mfa_url` 时先在冲突面板批量或逐条选择，再应用导入
